...
public final class InMemoryJavaFileManager extends ForwardingJavaFileManager<JavaFileManager> {
  private final LoadingCache<URI, JavaFileObject> inMemoryFileObjects = CacheBuilder.newBuilder().build(new CacheLoader<URI, JavaFileObject>() {
    @Override
  ...
  public InMemoryJavaFileManager(JavaFileManager fileManager) {
    super(fileManager);
    classLoader = new InMemoryClassLoader(this);
  }
  ...
  private static URI uriForFileObject(Location location, String packageName, String relativeName) {
    ...
    uri.append(toFileName(packageName, relativeName));
  ...
  ImmutableList<JavaFileObject> getGeneratedSources() {
    ...
    for (Entry<URI, JavaFileObject> entry : inMemoryFileObjects.asMap().entrySet()) {
      if (entry.getKey().getPath().startsWith("/" + StandardLocation.SOURCE_OUTPUT.name()) && (entry.getValue().getKind() == Kind.SOURCE)) {
  ...
  public ImmutableList<JavaFileObject> getOutputFiles() {
  ...
  public boolean hasLocation(Location location) {
    return location == StandardLocation.CLASS_PATH
        ...
        || location == StandardLocation.PLATFORM_CLASS_PATH; // we don't care about source and other location types - not needed for compilation
  }
  ...
  public Iterable<JavaFileObject> list(Location location, String packageName, Set<Kind> kinds, boolean recurse) throws IOException {
    ...
    if (location == StandardLocation.SOURCE_PATH) {
      for (Entry<URI, JavaFileObject> entry : inMemoryFileObjects.asMap().entrySet()) {
        if (entry.getKey().toString().contains(StandardLocation.SOURCE_PATH.getName())) {
          result.add(entry.getValue());
        }
      }
    }
    if (location == StandardLocation.CLASS_PATH) {
      for (Entry<URI, JavaFileObject> entry : inMemoryFileObjects.asMap().entrySet()) {
        if (entry.getKey().toString().contains("mem://" + StandardLocation.CLASS_PATH.getName())) {
          result.add(entry.getValue());
        }
      }
    }
  ...
  public static String toFileName(String packageName, String relativeName) {
  ...
  public static final class InMemoryJavaFileObject extends SimpleJavaFileObject implements JavaFileObject {
    private long lastModified = 0L;
    public Optional<ByteSource> data = Optional.absent();
