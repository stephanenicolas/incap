...
/** Retrieves annotation using the element query language. */
public class AnnotationFinder {
    ...
    /**
     * Retrieves an element given a query string. Query string
     *
     * @param query a query using the element query language.
     * @return the element specified by the query or null if it can't be found. @See
     *     #AnnotationFinder
     */
    public TypeElement lookupElement(String query) {
        ...
        switch (locator) {
            case LOCATOR_ANNOTATION_ON_FIELD:
                return lookupAnnotationOnField(
                        query.substring(LOCATOR_ANNOTATION_ON_FIELD.length() + 1));
            default:
    ...
    private TypeElement lookupAnnotationOnField(String subQuery) {
        ...
        final List<? extends AnnotationMirror> annotationMirrors =
                fieldElement.getAnnotationMirrors();
    ...
    private TypeElement annotationByTypeName(
            List<? extends AnnotationMirror> annotationMirrors, String annotationTypeName) {
        for (AnnotationMirror annotationMirror : annotationMirrors) {
            final String currentAnnotationTypeName =
